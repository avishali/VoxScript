                                             ...
                                             const juce::String& tempFilePrefix)
{
    ...
    const juce::int64 totalSourceSamples = reader->lengthInSamples;
    ...
    while (samplesRead < totalSourceSamples && !aborted)
    {
        ...
        const int numToRead = static_cast<int> (
            juce::jmin (static_cast<juce::int64> (CHUNK_SIZE), totalSourceSamples - samplesRead)
        );
...
juce::int64 AudioExtractor::getExpectedOutputSize (juce::ARAAudioSource* araSource)
{
    if (araSource == nullptr || !araSource->isSampleAccessEnabled())
        return 0;
    ...
    const double sourceRate = reader->sampleRate;
    const juce::int64 sourceSamples = reader->lengthInSamples;
    const double resampleRatio = TARGET_SAMPLE_RATE / sourceRate;
    
    return static_cast<juce::int64> (sourceSamples * resampleRatio);
}
